...
#ifdef DEBUG	
#define pmap_cprintf(...) cprintf(__VA_ARGS__)
#else
#define pmap_cprintf(...)
#endif
...
boot_alloc(uint32_t n)
{
	...
	if (!nextfree) {
		...
		pmap_cprintf("First initialize for the nextfree VA [%p]", nextfree);
		pmap_cprintf(": PA is [0x%x]\n", (int)(nextfree - KERNBASE));
	}
...
mem_init(void)
{
	...
	cprintf("\nUVPT = [0x%x];\nPDX(UVPT) = [%d: 0x%x];\nkern_pgdir[PDX(UVPY)]=[0x%x] with perm = [%p]\n",
		(int)UVPT, (int)PDX(UVPT), (int)PDX(UVPT), kern_pgdir[PDX(UVPT)], (int)(PTE_U | PTE_P));
...
mem_init_mp(void)
{
	...
	//
	// For CPU i, use the physical memory that 'fpercpu_kstacks[i]' refers
...
page_alloc(int alloc_flags)
{
	...
	if (page_free_list == NULL){
		pmap_cprintf("no enough space!!!!!!############################3\n");
		return NULL;
	}
	struct Page *free;
	free = page_free_list;
