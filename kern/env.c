...
#ifdef ENV_DEBUG	
#define env_cprintf(...) cprintf(__VA_ARGS__)
#else
#define env_cprintf(...)
#endif
...
load_icode(struct Env *e, uint8_t *binary, size_t size)
{
	...
	for(i = 0; i < elfhd->e_phnum; i++,ph++){
		...
		if(ph->p_type == ELF_PROG_LOAD){
			...
			env_cprintf("memmove() at ph->p_va[%p] with size[0x%x] succeed!\n",
				ph->p_va, ph->p_filesz);
